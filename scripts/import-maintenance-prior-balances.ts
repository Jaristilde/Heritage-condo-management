...
/**
 * Import Maintenance Prior Balances from CSV
 *
 * CSV Format:
 * Unit,Owner,Prior Balance (June 2025)
 * 201,Jose Peirats,1234.56
 *
 * This updates the maintenance_prior_balance field for each unit.
 */
...
async function importMaintenancePriorBalances() {
  console.log("📊 Importing Maintenance Prior Balances from CSV...\n");

  // Check if file exists
  const csvPath = path.join(process.cwd(), 'public/templates/maintenance-prior-balance-template.csv');

  if (!fs.existsSync(csvPath)) {
    console.error("❌ CSV file not found at:", csvPath);
    console.log("\n💡 Please create the file with this format:");
    console.log("Unit,Owner,Prior Balance (June 2025)");
  ...
  try {
    ...
    for (const row of rows) {
      try {
        ...
        // Update unit with prior balance
        await db
          .update(units)
          .set({
            maintenancePriorBalance: row.priorBalance,
          })
          .where(eq(units.unitNumber, row.unit));

        console.log(`✅ Unit ${row.unit}: Set prior balance to $${priorBalance.toFixed(2)}`);
        updated++;
    ...
    console.error("❌ Error importing prior balances:", error);
    throw error;
  }
...
importMaintenancePriorBalances();
